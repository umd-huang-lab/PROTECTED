...
def main(args):
    ...
    def dump_one_exp_id(best_exp_id):
        ...
        if best_exp_id is not None:
            ...
            path = f"best_model-{alg_name}-{env_name}.{best_exp_id[:8]}.model" if args.output is None else args.output
            if not args.all_ckpts:
                for j, sub_ckpts in enumerate(ckpts):
                    dump_model(sub_ckpts, index_id, path + f'_{j}')
            else:
                for j, sub_ckpts in enumerate(ckpts):
                    iters = sub_ckpts.df['iteration']
                    for i, it in enumerate(iters):
                        if i % args.dump_step != 0:
                            continue
                        dump_model(sub_ckpts, i, path + f'_{j}' + f'_{i}')
    ...
    # print("length: %d" % (len(all_exp_id)))
    # return len(all_exp_id)
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--base_directory', type=str, help='agent dir containing cox experiments', default='pa_atla_ppo_walker/agents/4b2bbadb-c603-4088-be0e-0ac11617ee05')
    ...
    if args.output == '':
        args.output = f'{args.base_directory}/{uuid_str}/model'
    main(args)
