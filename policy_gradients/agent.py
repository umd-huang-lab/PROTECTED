...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        self.policy_model_alias = self.policy_model
        ...
        if self.MODE == 'adv_ppo' or self.MODE == 'adv_trpo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo':
            ...
            self.params.ADV_POLICY_ADAM = optim.Adam(self.adversary_policy_model.parameters(), lr=self.ADV_PPO_LR_ADAM,
                                                     eps=1e-5)

            # Adversary value function.
            self.adversary_val_model = value_net_class(self.NUM_FEATURES, self.INITIALIZATION)
            self.adversary_val_model2 = value_net_class(self.NUM_FEATURES, self.INITIALIZATION)
            if not self.CPU:
                ...
                self.adversary_val_model2 = self.adversary_val_model2.to("cuda:{}".format(self.params.CUDA_ID))
            ...
            if self.ANNEAL_LR:
                ...
                adv_vs2 = optim.lr_scheduler.LambdaLR(self.adversary_val_opt2, lr_lambda=adv_lam)
                self.params.ADV_POLICY_SCHEDULER = adv_ps
                self.params.ADV_VALUE_SCHEDULER = adv_vs
                self.params.ADV_VALUE_SCHEDULER2 = adv_vs2
        ...
        self.meta_policy_list = []
        if self.params.iteration > 1 or self.params.attack_multiple_victims:
            self.initialize_past_victim_list()
            self.exp3 = Exp3Alg(K=len(self.past_victim_list), T=self.params.train_steps, normalizer=6000)
    ...
    def imit_steps(self, all_actions, all_states, all_not_dones, num_epochs):
        ...
        for e in range(num_epochs):
            ...
            if self.HISTORY_LENGTH > 0:
                loss = 0.0
                batches, alive_masks, time_masks, lengths = pack_history([all_states, all_actions], all_not_dones,
    ...
    def multi_actor_step(self, actions, envs):
        ...
        if self.CPU:
            ...
            data = list(
                map(lambda p: cu_tensorize(p, cuda_id=self.params.CUDA_ID), [normed_rewards, states, not_dones]))
            ...
            if is_advpolicy_training:
                ...
                if collect_adversary_trajectory or self.params.ADV_ADVERSARY_RATIO >= random.random():
                    if self.MODE == "adv_pa_ppo":
                        ...
                        next_adv_perturbation_log_probs = self.adversary_policy_model.get_loglikelihood(
                            adv_perturbation_pds, next_adv_perturbations)
    ...
    def collect_saps(self, num_saps, should_log=True, return_rewards=False,
                     should_tqdm=False, test=False, collect_adversary_trajectory=False, use_first_model=True):
                    ...
                    if collect_adversary_trajectory:
                        if self.HISTORY_LENGTH > 0 and self.USE_LSTM_VAL:
                            if use_first_model:
                                ...
                                values = self.adversary_val_model2(trajs.states, trajs.not_dones).squeeze(-1)
                        else:
                            if use_first_model:
                                values = self.adversary_val_model(trajs.states).squeeze(-1)
                            else:
                                values = self.adversary_val_model2(trajs.states).squeeze(-1)
                    else:
                        if self.HISTORY_LENGTH > 0 and self.USE_LSTM_VAL:
    ...
    def sarsa_steps(self, saps):
        ...
        for i in range(self.params.VAL_EPOCHS):
            ...
            for selected in splits:
                ...
                sel_states, sel_actions, sel_rewards, sel_not_dones = sel(saps.states, saps.actions, saps.rewards,
                                                                          saps.not_dones)
                ...
                bounded_inputs = BoundedTensor(inputs, ptb=PerturbationLpNorm(norm=np.inf,
                                                                              eps=self.sarsa_eps_scheduler.get_eps()))
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True,
        ...
        if adversary_step:
            ...
            if self.ANNEAL_LR:
                ...
                val_scheduler2 = self.ADV_VALUE_SCHEDULER2
            ...
            val_model2 = self.adversary_val_model2
            val_opt2 = self.adversary_val_opt2
        ...
        if not self.SHARE_WEIGHTS:
            ...
            # TODO: new val
            if saps2 is not None:
                val_loss2 = value_step(saps2.states, saps2.returns,
                                       saps2.advantages, saps2.not_dones, val_model2,
                                       val_opt2, self.params, store_to_pass,
                                       old_vs=saps2.values.detach()).mean()
        ...
        if logging:
            if saps2 is not None:
                self.store.log_table_and_tb('optimization' + table_name_suffix, {
                    ...
                    'final_value_loss2': val_loss2
                })
            else:
                if table_name_suffix == '_adv':
                    self.store.log_table_and_tb('optimization' + table_name_suffix, {
                        'final_value_loss': val_loss,
                        'final_value_loss2': 0
                    })
        ...
        if (
                self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo') and not adversary_step and logging:
            # Logging Robust PPO KL, entropy, etc.
            store_to_pass = self.store

        # Take optimizer steps
        if saps2 is not None:
            self.merge_saps(saps, saps2)
        ...
        if (
                self.params.MODE == 'robust_ppo' or self.params.MODE == 'adv_sa_ppo' or self.params.MODE == 'adv_pa_ppo') and not adversary_step:
            ...
            if self.PPO_LR_ADAM != 0.0:
                for i in range(int(self.ADV_POLICY_STEPS)):
                    avg_ep_reward = self.train_step_impl(adversary_step=False,
                                                         increment_scheduler=(i == self.ADV_POLICY_STEPS - 1))
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        avg_ep_reward = 0
        should_log_for_attacker = (not adversary_step) or (not self.params.attack_multiple_victims)
        if not self.params.attack_multiple_victims:
            saps, avg_ep_reward, avg_ep_length = self.collect_saps(num_saps,
                                                                   collect_adversary_trajectory=adversary_step,
                                                                   should_log=should_log_for_attacker)
        if (self.params.iteration > 1 and len(self.past_victim_list) > 0 and adversary_step) or self.params.attack_multiple_victims:
            if not self.params.attack_exp3:
                if self.n_steps % 10 == 0:
                    tmp_avg_ep_reward_list = []
                    for i in range(len(self.past_victim_list)):
                        self.set_past_victim(i)
                        _, tmp_avg_ep_reward, _ = self.collect_saps(num_saps, collect_adversary_trajectory=True,
                                                                    should_log=False)
                        tmp_avg_ep_reward_list.append(tmp_avg_ep_reward)
                    self.best_victim = np.argmax(np.array(tmp_avg_ep_reward_list))
            else:
                self.best_victim = self.exp3.sample()
                meta_policy = self.exp3.get_policy()
                self.meta_policy_list.append(meta_policy)
                print(f"sampled victim policy {self.best_victim} from meta policy {meta_policy}")
            self.set_past_victim(self.best_victim)
            past_victim_saps, tmp_rew, _ = self.collect_saps(num_saps, collect_adversary_trajectory=adversary_step,
                                                       use_first_model=False, should_log=True)
            if self.params.attack_exp3:
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None):
        ...
        with torch.no_grad():
            ...
            if compute_bounds:
                ...
                eps = float(self.params.ROBUST_PPO_EPS) if self.params.ATTACK_EPS == "same" else float(
                    self.params.ATTACK_EPS)
    ...
    def agent_from_data(store, row, cpu, extra_params=None, override_params=None, excluded_params=None):
        ...
        if override_params is not None:
            for k in override_params.keys():
                if k not in excluded_params and override_params[k] is not None and override_params[k] != agent_params[
                    k]:
    ...
    def start_new_iteration(self):
        self.add_victim(path=None)
        self.n_steps = 0

        self.policy_model = self.policy_net_class(self.NUM_FEATURES, self.NUM_ACTIONS,
                                                  self.INITIALIZATION,
                                                  time_in_state=self.time_in_state,
                                                  activation=self.policy_activation)
        if len(self.past_victim_list) > 0:
            self.policy_model.load_state_dict(self.past_victim_list[-1].state_dict())
        self.policy_model_alias = self.policy_model
        ...
        # Adversary value function.
        self.adversary_val_model = self.value_net_class(self.NUM_FEATURES, self.INITIALIZATION)
        self.adversary_val_model2 = self.value_net_class(self.NUM_FEATURES, self.INITIALIZATION)
        if not self.CPU:
            self.adversary_policy_model = self.adversary_policy_model.to("cuda:{}".format(self.params.CUDA_ID))
            self.adversary_val_model = self.adversary_val_model.to("cuda:{}".format(self.params.CUDA_ID))
            self.adversary_val_model2 = self.adversary_val_model2.to("cuda:{}".format(self.params.CUDA_ID))
        ...
        # Value function optimization
        self.val_model = self.value_net_class(self.NUM_FEATURES, self.INITIALIZATION)
        if not self.CPU:
            self.val_model = self.val_model.to("cuda:{}".format(self.params.CUDA_ID))
    ...
    def initialize_past_victim_list(self):
        self.past_victim_list = []
        self.past_victim_envs_list = []
        for model in self.params.ref_model_list:
            self.add_victim(model)
        if len(self.past_victim_list) > 0:
            self.policy_model.load_state_dict(self.past_victim_list[-1].state_dict())

    def load_ref_model_list(self):
        self.past_victim_list = []
        self.past_victim_envs_list = []
        for model_path in self.params.ref_model_list:
            policy_model = self.load_past_victim(model_path, load_env=True)
            policy_model.log_stdev.data[:] = -100
            self.past_victim_list.append(policy_model)

    def add_victim(self, path):
        self.past_victim_list.append(self.load_past_victim(path))

    def load_past_victim(self, path, load_env=False):
        policy_model = self.policy_net_class(self.NUM_FEATURES, self.NUM_ACTIONS,
                                             self.INITIALIZATION,
                                             time_in_state=self.time_in_state,
                                             activation=self.policy_activation)
        if path is None:
            policy_model.load_state_dict(self.policy_model.state_dict())
        else:
            pretrained_model = torch.load(path)
            policy_model.load_state_dict(pretrained_model['policy_model'])
            if self.params.attack_multiple_victims or load_env:
                self.past_victim_envs_list.append(pretrained_model['envs'])
        # policy_model.log_stdev.data[:] = -100
        return policy_model

    def set_past_victim(self, i):
        self.policy_model = self.past_victim_list[i]
        if self.params.attack_multiple_victims:
            self.envs = self.past_victim_envs_list[i]

    def reset_to_current_victim(self):
        self.policy_model = self.policy_model_alias
        self.envs = self.envs_alias
